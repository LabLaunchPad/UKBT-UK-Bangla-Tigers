...
| CLIENT_REQ_008 | EV-027 | Exclude "MD Siraj Ullah Khadem Nipo" from the squad list | Content correction | HIGH | Players Profile / Uppsala Tigers page | **Cross-referenced, EV-030:** an "Overseas Signings — Uppsala Tigers" graphic names "Nipo Khadem" (Portugal) — very likely the same person, shortened form | None | Exclude "Nipo Khadem"/"MD Siraj Ullah Khadem Nipo" from any published Uppsala Tigers or UKBT roster | **APPLIED going forward** — the full authoritative squad list (beyond this one graphic) still hasn't been supplied. **SCOPED WAIVER 2026-09-11 (owner chat):** the `.jpg` variant of that graphic (`nordic-smash-slide.jpg`) renders as the `/franchises` banner backdrop with the baked-in name visible. Rosters, DOM copy, alt text, and the `.webp` sibling remain excluded — waiver covers this banner use only. |
